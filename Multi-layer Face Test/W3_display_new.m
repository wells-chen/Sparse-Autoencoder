function  W3_display_new(W11,W2,W3,maxpooling_size1,maxpooling_size2)
...
for i=1:M
    ...
    for k=1:K*maxpooling_size1
        for l=1:L*maxpooling_size1
            temp_max= W2_max_cell{i,1}(k,l);layer=1;
            for t=2:N1
                if temp_max < W2_max_cell{i,t}(k,l)
                    temp_max=W2_max_cell{i,t}(k,l);layer=t;
                end
            end
            W2_temp(k:k+filter_size-1 ,l:l+filter_size-1) = temp_max*(reshape(W11(:,layer),filter_size ,filter_size))+W2_temp(k:k+filter_size-1 ,l:l+filter_size-1);
        end
    end
   ...
   %N1 第一层滤波器个数
    ...
    for k=1:K*maxpooling_size2
        for l=1:L*maxpooling_size2
            temp_max= W3_max_cell{i,1}(k,l);layer=1;
            for t=2:N1
                if temp_max < W3_max_cell{i,t}(k,l)
                    temp_max=W3_max_cell{i,t}(k,l);layer=t;
                end
            end
            W3_temp(k:k+filter_size-1 ,l:l+filter_size-1) = temp_max*W2_backed{layer}+W3_temp(k:k+filter_size-1 ,l:l+filter_size-1);
        end
    end
...
display_network(W3_toshow,12);
filename =['Filter','W3_max_Display,'.','jpg'];
