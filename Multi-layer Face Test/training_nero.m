...
for i=1:size(data_culve2_test,2)
    temp(5:m+4,5:n+4,:)=data_culve2_test{i}(:,:,:);
    data_culve2_test_extend{i} = temp;
end
clear temp m n k;
...
save 'data_save\org_W3_200\data_culve3_no_noise_p' data_culve3_test;
...
for i=1:size(data_culve2_test_n,2)
    temp(5:m+4,5:n+4,:)=data_culve2_test_n{i}(:,:,:);
    data_culve2_test_n_extend{i}= temp;
end
clear temp m n k;
