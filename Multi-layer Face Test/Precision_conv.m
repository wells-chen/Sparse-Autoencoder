function [R,P,opt_location] = Precision_conv(training_data1) 
%evaluation  the training data
M=size(training_data1,2);
[S,K,L]=size(training_data1{1,1});
training_data{1,M}{1,S}=[];
temp= zeros(K,L);
for i=1:M
    for j=1:S
        temp=training_data1{i}(j,:,:);
       training_data{1,i}{1,j}=temp;
    end
end
clear temp;
N =S;
...
for i=1:M
    ...
    Xmin = 10^5;
    for j=1:N
        temp1 = training_data{1,i}{1,j};
        temp2 =max(temp1(:));
        temp3 = min(temp1(:));
        Xmax = max(temp2,Xmax);
        Xmin = min(temp3,Xmin);
        training_data{1,i}{1,j} = (training_data{1,i}{1,j}-Xmin)./(Xmax-Xmin);
    end      
end
clear temp1 temp2 temp3 Xmax Xmin;
%求正负样本数据集
